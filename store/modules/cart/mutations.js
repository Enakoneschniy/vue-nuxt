import { ADD_TO_CART, REMOVE_FROM_CART, CHANGE_COUNT } from "./mutation-types";

export default {
  [ADD_TO_CART](store, product) {
    const index = store.items.findIndex(p => p.id === product.id);
    if (index !== -1) {
      store.items.splice(index, 1, Object.assign({}, store.items[index], { count: store.items[index].count + 1 }));
    } else {
